...
### 6.7.1 Boss 防守反击演出（升龙挥空惩罚）

玩家升龙被 Boss 闪避躲开（挥空）时触发 **Boss 防守反击**，视觉要点：

- **阶段主题**：切入危机红（`PHASE.BOSSRETALIATE` 主题，bg 深红 + 红 rim light + 红色渐晕），与「抢占先机」金蓝、反击青形成区分。
- **Boss 残影冲刺**：进入窗口瞬间，Boss 沿冲刺路径生成 2~3 个半透明残影（复用 `spawnGhostTrailFor('boss')`）冲向玩家，配合 0.5 倍慢动作营造压迫感。
- **防守圆环**：屏幕居中红色/金色扩散圆环，文案提示"🛡️ 做闪避💨 / 石头🪨化解！"；填满前未化解则 Boss 出拳命中玩家。
- **命中演出**：复用 `applyPreemptFailDmg` —— Boss 出拳、玩家红色震动 + 受击闪红 +「💥 Boss 反击！」飘字，扣 2 点。
- **化解演出**：玩家做闪避/石头防御，绿色爆点 +「🛡️ 化解成功！」飘字，无伤。
