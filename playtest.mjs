// 自动化 playtest：针对「统一闪避系统」的规则验证 + 完整对局回归
...
console.log('\n========== 一、闪避 vs 升龙（核心） ==========');
{
  ...
  check('闪避vs升龙(有架势)：扣1架势(2→1)', j.pCharge === 1, `pCharge=${j.pCharge}`);
...
{
  let j = await single(D, [C], { p: 0, b: 0, randVal: 0.9 }); // 无架势→失败
  check('闪避vs升龙(无架势,失败)：攻击-1', j.pMod === -1, `pMod=${j.pMod}`);
  check('闪避vs升龙(无架势,失败)：玩家受升龙伤害(10→8)', j.pHp === 8, `pHp=${j.pHp}`);
  check('闪避vs升龙(无架势,失败)：无架势不扣(仍0)', j.pCharge === 0, `pCharge=${j.pCharge}`);
}
{
  let j = await single(D, [C], { p: 0, b: 0, randVal: 0.1 }); // 无架势→成功
  check('闪避vs升龙(无架势,成功)：攻击+1', j.pMod === 1, `pMod=${j.pMod}`);
  check('闪避vs升龙(无架势,成功)：不掉血', j.pHp === 10, `pHp=${j.pHp}`);
  check('闪避vs升龙(无架势,成功)：无架势不扣(仍0)', j.pCharge === 0, `pCharge=${j.pCharge}`);
}

console.log('\n========== 二、闪避 vs 剪刀（核心） ==========');
{
  let j = await single(D, [S], { p: 3, b: 0 }); // 有架势必成
  check('闪避vs剪刀(有架势)：扣1架势(3→2)', j.pCharge === 2, `pCharge=${j.pCharge}`);
  check('闪避vs剪刀(有架势)：攻击+1', j.pMod === 1, `pMod=${j.pMod}`);
  check('闪避vs剪刀(有架势)：不掉血', j.pHp === 10, `pHp=${j.pHp}`);
}
{
  let j = await single(D, [S], { p: 0, b: 0, randVal: 0.9 }); // 无架势→失败
...
console.log('\n========== 三、Boss 闪避 vs 玩家剪刀（对称） ==========');
{
  ...
  check('Boss闪避(有架势)：Boss扣1架势(2→1)', j.bCharge === 1, `bCharge=${j.bCharge}`);
  check('Boss闪避(有架势)：Boss攻击+1', j.bMod === 1, `bMod=${j.bMod}`);
  check('Boss闪避成功：Boss不掉血', j.bHp === 20, `bHp=${j.bHp}`);
}
{
  let j = await single(S, [D], { p: 0, b: 0, randVal: 0.9 }); // Boss无架势→失败
